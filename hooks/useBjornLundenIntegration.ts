...
export const useBjornLundenIntegration = (tenantId: string | undefined) => {
  const [isLoading, setIsLoading] = useState(false);
  const [isConnecting, setIsConnecting] = useState(false);
  const [isSyncing, setIsSyncing] = useState(false);
  const [isDisconnecting, setIsDisconnecting] = useState(false);
  const [status, setStatus] = useState<BjornLundenStatus | null>(null);
  ...
  // Check Björn Lundén connection status
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${SUPABASE_URL}/functions/v1/bjorn-lunden-oauth?action=status&tenant_id=${tenantId}`,
        { headers }
      );
  ...
  const startOAuth = useCallback(async (): Promise<string | null> => {
    ...
    try {
      const headers = await getAuthHeaders();
      const response = await fetch(
        `${SUPABASE_URL}/functions/v1/bjorn-lunden-oauth?action=authorize`,
        {
          method: 'POST',
          headers,
  ...
  const syncData = useCallback(async (): Promise<SyncResult | null> => {
    ...
    try {
      const headers = await getAuthHeaders();
      const response = await fetch(
        `${SUPABASE_URL}/functions/v1/bjorn-lunden-sync?tenant_id=${tenantId}&action=full`,
        { headers }
      );
  ...
  const disconnect = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const headers = await getAuthHeaders();
      const response = await fetch(
        `${SUPABASE_URL}/functions/v1/bjorn-lunden-oauth?action=disconnect`,
        {
          method: 'POST',
          headers,
          body: JSON.stringify({ tenant_id: tenantId }),
...
export default useBjornLundenIntegration;
